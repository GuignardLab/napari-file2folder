...
class File2FolderWidget(QWidget):
    ...
    def __init__(self, viewer: "napari.viewer.Viewer"):
        ...
        self._array_file_path.label_changed.connect(
        ...
        self._dimension_choice_combo = create_widget(
            widget_type="ComboBox", options={"nullable": False}
        )
        ...
        self._save_to_folder_button.clicked.connect(self._save_to_folder)
        ...
        self.layout().addWidget(
            QLabel(f"Dimensions of currently selected layer:")
        )
        ...
        self.layout().addWidget(
            QLabel("<u>(optional) Inspect middle element:</u>")
        )
        self.layout().addWidget(middle_elem_button_container.native)
        self.layout().addWidget(QLabel(""))

        self.layout().addWidget(
            QLabel("<u>Select path where the folder will be created:</u>")
        )
    ...
    def _save_to_folder(self):
        save_path, save_path_valid = self._coerce_path(
            self._save_to_folder_path.value, "folder"
        )
        if save_path_valid:
            path, path_valid = self._coerce_path(
                self._array_file_path.value, "file"
            )
            if path_valid:

                dimension_index, dimension_shape = (
                    self._dimension_index_from_str(
                        self._dimension_choice_combo.value
                    )
                )

                path_to_folder = (
                    f"{save_path}/{path.stem}_dim{dimension_index}"
                )
    ...
    def _lazy_save_slices(
        ...
        if (
            ...
            with tifffile.TiffFile(file) as tif:
                ...
                dim_size = shape[slice_dim]

                # memory-mapped lazy access (does NOT fully load into RAM)
                data = series.asarray(out="memmap")

                for element_index in range(dim_size):
                    slices[slice_dim] = element_index

                    stack = data[tuple(slices)]
                    ...
                    self._progress_bar.setValue(
                        int((element_index + 1) / dim_size * 100)
                    )
            # with tifffile.TiffFile(file) as tif:
    ...
    def _lazy_grab_slice(self, slice_dim, file, element_index: int = None):
        ...
        if (
            ...
            with tifffile.TiffFile(file) as tif:
                ...
                slices = [slice(None)] * series.ndim
                slices[slice_dim] = element_index

                stack = series.asarray(out="memmap")[tuple(slices)]
    ...
    def _update_dimensions(self):
        ...
        if path_valid:
            ...
            self._shape_text.setText(
                f"Shape: <a style=color:#1A85FF;>{shape}</a>"
            )
        else:
    ...
    def _update_layer_combos(self):
        ...
        all_choices = [
            self._array_layer_combo.native.itemText(i)
            for i in range(self._array_layer_combo.native.count())
        ]
